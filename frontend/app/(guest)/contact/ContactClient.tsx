...
import { Company, companyApi, contactApi } from '@/lib/publicServiceApi';
...
function validateForm(form: FormData): Record<string, string> {
  ...
  else if (form.name.length > 255) errs.name = 'Name must not exceed 255 characters.';
  if (!form.email.trim()) errs.email = 'Email is required.';
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) errs.email = 'Please enter a valid email address.';
  else if (form.email.length > 255) errs.email = 'Email must not exceed 255 characters.';
  if (!form.subject.trim()) errs.subject = 'Subject is required.';
  else if (form.subject.trim().length < 3) errs.subject = 'Subject must be at least 3 characters.';
  else if (form.subject.length > 255) errs.subject = 'Subject must not exceed 255 characters.';
  if (!form.message.trim()) errs.message = 'Message is required.';
  else if (form.message.trim().length < 10) errs.message = 'Message must be at least 10 characters.';
  else if (form.message.length > 5000) errs.message = 'Message must not exceed 5000 characters.';
  return errs;
}
    ...
    <div className="pt-16 min-h-screen bg-white dark:bg-gray-950">
      ...
      <section className="relative bg-gray-800 text-white pb-24 pt-36 text-center overflow-hidden">
        ...
        <div className="relative z-10">
          <p className="text-amber-400 uppercase tracking-widest text-sm mb-3 font-medium">Get In Touch</p>
        ...
        <div>
          ...
          <div className="space-y-4">
            ...
            {loading ? (
              <div className="flex items-start gap-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-xl">
                ...
                <div className="flex-1 space-y-2 py-1"><Skeleton className="h-3 w-16" /><Skeleton className="h-4 w-48" /></div>
              </div>
            ) : company?.address ? (
              <div className="flex items-start gap-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-xl">
            ...
            {loading ? (
              <div className="flex items-start gap-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-xl">
                <Skeleton className="w-10 h-10 rounded-lg shrink-0" />
                <div className="flex-1 space-y-2 py-1"><Skeleton className="h-3 w-16" /><Skeleton className="h-4 w-32" /><Skeleton className="h-4 w-28" /></div>
              </div>
            ) : phones.length > 0 ? (
              <div className="flex items-start gap-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-xl">
                <div className="w-10 h-10 bg-amber-100 dark:bg-amber-900/30 rounded-lg flex items-center justify-center shrink-0">
                ...
                <div>
                  <p className="text-xs text-gray-400 font-medium uppercase tracking-wide mb-1">{phones.length > 1 ? 'Phone Numbers' : 'Phone'}</p>
                  <div className="space-y-1">
                    {phones.map((p, i) => (
                      <a key={i} href={`tel:${p}`} className="block text-sm font-medium text-gray-800 dark:text-gray-200 hover:text-amber-500 dark:hover:text-amber-400 transition-colors">{p}</a>
                    ))}
                  </div>
                </div>
              </div>
            ...
            {loading ? (
              <div className="flex items-start gap-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-xl">
                <Skeleton className="w-10 h-10 rounded-lg shrink-0" />
                <div className="flex-1 space-y-2 py-1"><Skeleton className="h-3 w-16" /><Skeleton className="h-4 w-40" /><Skeleton className="h-4 w-36" /></div>
              </div>
            ) : emails.length > 0 ? (
              <div className="flex items-start gap-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-xl">
                <div className="w-10 h-10 bg-amber-100 dark:bg-amber-900/30 rounded-lg flex items-center justify-center shrink-0">
                ...
                <div>
                  <p className="text-xs text-gray-400 font-medium uppercase tracking-wide mb-1">{emails.length > 1 ? 'Email Addresses' : 'Email'}</p>
                  <div className="space-y-1">
                    {emails.map((em, i) => (
                      <a key={i} href={`mailto:${em}`} className="block text-sm font-medium text-gray-800 dark:text-gray-200 hover:text-amber-500 dark:hover:text-amber-400 transition-colors">{em}</a>
                    ))}
                  </div>
                </div>
              </div>
          ...
          <div className="mt-6 rounded-2xl overflow-hidden h-52 border border-gray-100 dark:border-gray-800">
            {loading ? (
              ...
              <iframe src={company.location_url} className="w-full h-full" loading="lazy" allowFullScreen referrerPolicy="no-referrer-when-downgrade" />
            ) : (
              <div className="w-full h-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-gray-400 gap-2">
                <MapPin size={24} /><span className="text-sm">No location set</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-8">
          {sent ? (
            <div className="text-center py-12 flex flex-col items-center gap-4">
              ...
              <button onClick={() => { setSent(false); setForm({ name: '', email: '', subject: '', message: '' }); setTouched({}); setErrors({}); }} className="mt-2 text-sm text-amber-500 hover:underline">
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" placeholder="Your name" value={form.name} onChange={handleChange('name')} onBlur={handleBlur('name')} className={inputCls(errors.name)} />
                ...
                <div>
                  ...
                  <input type="email" placeholder="you@email.com" value={form.email} onChange={handleChange('email')} onBlur={handleBlur('email')} className={inputCls(errors.email)} />
              ...
              <div>
                ...
                <input type="text" placeholder="How can we help?" value={form.subject} onChange={handleChange('subject')} onBlur={handleBlur('subject')} className={inputCls(errors.subject)} />
                {errors.subject && <p className="text-xs text-red-500 mt-1">{errors.subject}</p>}
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Message <span className="text-red-500">*</span></label>
                <textarea rows={5} placeholder="Your message..." value={form.message} onChange={handleChange('message')} onBlur={handleBlur('message')} className={`${inputCls(errors.message)} resize-none`} />
