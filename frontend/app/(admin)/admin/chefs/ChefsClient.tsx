...
import ShowChefModal from '@/components/admin/chefs/ShowChefModal';
import CreateChefModal from '@/components/admin/chefs/CreateChefModal';
import EditChefModal from '@/components/admin/chefs/EditChefModal';
import DeleteChefModal from '@/components/admin/chefs/DeleteChefModal';


type SortKey = 'fullname' | 'position' | 'created_at';
...
export default function ChefsClient() {
  ...
  const [editTarget, setEditTarget]                 = useState<Chef | null>(null);
  const [deleteTarget, setDeleteTarget]             = useState<Chef | null>(null);
  const [showTarget, setShowTarget]                 = useState<Chef | null>(null);
  ...
  const fetchChefs = useCallback(async () => {
    setLoading(true);
    try {
      const res = await chefApi.getAll();
      setChefs(res.data);
    } catch {
  ...
  const processed = useMemo(() => {
    ...
    return data;
  }, [chefs, search, statusFilter, sortKey, sortDir]);
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      await chefApi.deleteSelected(selected);
      toast.success(`${selected.length} chefs deleted.`);
      setSelected([]);
      setShowDeleteSelected(false);
      fetchChefs();
    } catch {
      toast.error('Failed to delete selected chefs.');
    } finally {
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      await chefApi.deleteAll();
      toast.success('All chefs deleted.');
      setSelected([]);
      setShowDeleteAll(false);
      fetchChefs();
    } catch {
      toast.error('Failed to delete all chefs.');
    } finally {
  ...
  return (
    <div className="">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Chefs</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{chefs.length} total chefs</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => setShowDeleteAll(true)} disabled={chefs.length === 0}
              ...
              <button key={s} onClick={() => { setStatusFilter(s); setPage(1); }}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                  statusFilter === s
                    ...
                    : s === 'inactive' ? 'bg-gray-400 dark:bg-gray-600 text-white shadow-sm'
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-3">
            ...
            <p className="text-sm text-gray-400">Loading chefs...</p>
          </div>
        ) : pageItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-3">
            ...
            <p className="text-gray-500 dark:text-gray-400 font-medium">No chefs found</p>
            {search && <p className="text-sm text-gray-400">Try adjusting your search or filters</p>}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
                    ...
                    <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Photo</th>
                    ...
                    <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider cursor-pointer select-none hidden md:table-cell" onClick={() => toggleSort('position')}>
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-gray-800/50">
                  {pageItems.map(chef => (
                    <tr key={chef.id} className={`hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors ${selected.includes(chef.id) ? 'bg-brand-50/50 dark:bg-brand-900/10' : ''}`}>
                      <td className="px-5 py-3.5">
                        <input type="checkbox" checked={selected.includes(chef.id)} onChange={() => toggleOne(chef.id)}
                          className="rounded border-gray-300 dark:border-gray-600 text-brand-500 focus:ring-brand-400 cursor-pointer" />
                      </td>
                      {/* Photo */}
                      <td className="px-5 py-3.5">
                        {chef.image_url ? (
                          <img src={chef.image_url} alt={chef.fullname} className="w-10 h-10 rounded-xl object-cover border border-gray-100 dark:border-gray-700" />
                        ) : (
                      ...
                      {/* Name */}
                      <td className="px-5 py-3.5 font-semibold text-gray-900 dark:text-white max-w-[140px] truncate">{chef.fullname}</td>
                      {/* Position */}
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => setShowTarget(chef)}
                          ...
                          <button onClick={() => setEditTarget(chef)}
                          ...
                          <button onClick={() => setDeleteTarget(chef)}
            ...
            <div className="px-5 py-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/30">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                <p className="text-xs text-gray-400">
                  ...
                  of <span className="font-semibold text-gray-600 dark:text-gray-300">{processed.length}</span> chefs
      ...
      {/* ── Modals ── */}
      <ShowChefModal
        open={!!showTarget} chef={showTarget}
      ...
      <CreateChefModal
        ...
        onSuccess={() => { toast.success('Chef created successfully!'); fetchChefs(); }}
      />
      <EditChefModal
        open={!!editTarget} chef={editTarget}
        onClose={() => setEditTarget(null)}
        onSuccess={() => { toast.success('Chef updated successfully!'); fetchChefs(); }}
      />
      <DeleteChefModal
        open={!!deleteTarget} chef={deleteTarget}
      ...
      <ConfirmModal
        ...
        description={<>Are you sure you want to delete <span className="font-semibold text-gray-900 dark:text-white">{selected.length} chefs</span>? This action <span className="text-red-500 font-medium">cannot be undone</span>.</>}
      />
      <ConfirmModal
        open={showDeleteAll} title="Delete All Chefs" confirmLabel="Delete All" loading={deleteAllLoading}
        onConfirm={handleDeleteAll} onClose={() => setShowDeleteAll(false)}
        description={<>Are you sure you want to delete <span className="font-semibold text-gray-900 dark:text-white">all {chefs.length} chefs</span>? This action <span className="text-red-500 font-medium">cannot be undone</span> and will remove all associated photos.</>}
      />
    </div>
  );
}