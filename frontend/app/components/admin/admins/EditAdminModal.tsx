...
export default function EditAdminModal({ open, user, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await adminApi.update(user.id, fd);
      onSuccess();
      onClose();
    } catch (err: unknown) {
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-lg bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800 shrink-0">
            <div>
              <h2 className="font-bold text-lg text-gray-900 dark:text-white">Edit Admin</h2>
              <p className="text-xs text-gray-400 mt-0.5">Editing: {user.fullname}</p>
            </div>
          ...
          <div className="overflow-y-auto flex-1">
            <form onSubmit={handleSubmit} id="edit-admin-form" className="p-6 space-y-5">
              {errors.api && (
          ...
          <div className="flex gap-3 px-6 py-4 border-t border-gray-100 dark:border-gray-800 shrink-0">
            ...
            <button type="submit" form="edit-admin-form" disabled={loading}
