...
import { userApi, User } from '@/lib/adminServiceApi';
...
export default function ShowUserModal({ open, user, onClose, onStatusChange }: Props) {
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      toast.success(`User ${res.data.status ? 'activated' : 'deactivated'} successfully.`);
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-lg bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800 shrink-0">
            <h2 className="font-bold text-lg text-gray-900 dark:text-white">User Details</h2>
          ...
          {/* Avatar banner */}
          <div className="relative h-44 bg-gradient-to-br from-brand-500/20 via-brand-400/10 to-transparent dark:from-brand-900/40 dark:via-brand-900/20 dark:to-transparent overflow-hidden shrink-0 flex items-center justify-center">
            <div className="absolute -top-6 -right-6 w-32 h-32 rounded-full bg-brand-500/10 dark:bg-brand-500/5" />
            <div className="absolute -bottom-8 -left-8 w-40 h-40 rounded-full bg-brand-500/5 dark:bg-brand-500/5" />
            ...
            {user.avatar_url ? (
              ...
              <div className="relative z-10 w-20 h-20 rounded-2xl bg-brand-500 flex items-center justify-center border-4 border-white dark:border-gray-800 shadow-xl">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <span className="inline-flex items-center px-2.5 py-1 rounded-lg bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 text-xs font-semibold capitalize">
                    {user.role}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
                <div className="flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
                    user.status ? 'bg-emerald-50 dark:bg-emerald-900/20' : 'bg-red-50 dark:bg-red-900/20'
                  }`}>
                    {user.status
                  ...
                  <div>
                    ...
                    <p className={`text-xs font-medium mt-0.5 ${user.status ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}>
                      {user.status ? 'Active — can access the platform' : 'Inactive — access is blocked'}
                    </p>
                  </div>
                </div>
                <button
                  ...
                  className={`relative w-11 h-6 rounded-full transition-colors disabled:opacity-60 shrink-0 ${
                    user.status ? 'bg-emerald-500' : 'bg-gray-300 dark:bg-gray-600'
                  }`}>
                  {toggling ? (
