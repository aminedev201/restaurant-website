...
import { X, Calendar, Clock, ImageOff, Tag, AlignLeft, Edit2, Trash2, Percent, ToggleLeft, ToggleRight, FolderOpen, FileText } from 'lucide-react';
import { Plate } from '@/lib/adminServiceApi';
...
export default function ShowPlateModal({ open, plate, onClose, onEdit, onDelete }: Props) {
  if (!open || !plate) return null;

  const handleEdit   = () => { onClose(); onEdit(plate); };
  const handleDelete = () => { onClose(); onDelete(plate); };

  const finalPrice = plate.discount
    ? plate.price - (plate.price * plate.discount) / 100
    : plate.price;
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-lg bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800 shrink-0">
            <h2 className="font-bold text-lg text-gray-900 dark:text-white">Plate Details</h2>
          ...
          <div className="relative h-52 bg-gray-100 dark:bg-gray-800 overflow-hidden shrink-0">
            {plate.image_url ? (
              <>
                <div className="absolute inset-0 scale-110 blur-md opacity-40"
                  style={{ backgroundImage: `url(${plate.image_url})`, backgroundSize: 'cover', backgroundPosition: 'center' }} />
                <img src={plate.image_url} alt={plate.name} className="relative z-10 w-full h-full object-contain" />
              </>
            ) : (
            ...
            {plate.discount ? (
              <div className="absolute bottom-3 right-3 z-20">
                <span className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-red-500 text-white text-xs font-bold">
                   {plate.discount}<Percent size={11} /> OFF
                </span>
              </div>
            ) : null}
          </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="font-semibold text-gray-900 dark:text-white text-base">{plate.name}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-0.5">Category</p>
                  <p className="text-sm font-medium text-gray-700 dark:text-gray-300">{plate.category?.name ?? '—'}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                  ...
                  <p className="text-sm font-bold text-gray-900 dark:text-white">{plate.price.toFixed(2)} MAD</p>
                </div>
                {plate.old_price != null && (
                  <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                    <p className="text-xs text-gray-400 uppercase tracking-wide mb-1">Old Price</p>
                    <p className="text-sm font-medium text-gray-400 line-through">{plate.old_price.toFixed(2)} MAD</p>
                  </div>
                )}
                {plate.discount ? (
                  <div className="p-3 bg-red-50 dark:bg-red-900/20 rounded-xl">
                    <p className="text-xs text-red-400 uppercase tracking-wide mb-1">Final</p>
                    <p className="text-sm font-bold text-red-600 dark:text-red-400">{finalPrice.toFixed(2)} MAD</p>
                  </div>
                ) : null}
              </div>
              ...
              {plate.short_desc && (
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-0.5">Short Description</p>
                    <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed break-words">{plate.short_desc}</p>
                  </div>
                </div>
              )}
              ...
              {plate.desc && (
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-0.5">Full Description</p>
                    <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed whitespace-pre-line break-words">{plate.desc}</p>
                  </div>
                </div>
              )}
                ...
                <div className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      {new Date(plate.created_at).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium text-gray-700 dark:text-gray-300">
                      {new Date(plate.updated_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
                    </p>
                    <p className="text-xs text-gray-400">
                      {new Date(plate.updated_at).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}
                    </p>
                  </div>
                </div>
              </div>
