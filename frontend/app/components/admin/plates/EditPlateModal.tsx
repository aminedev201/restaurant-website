...
import { plateApi, categoryApi, Category, Plate } from '@/lib/adminServiceApi';
...
export default function EditPlateModal({ open, plate, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    if (plate) {
      setName(plate.name);
      setCategoryId(String(plate.category_id));
      setShortDesc(plate.short_desc ?? '');
      setDesc(plate.desc ?? '');
      setPrice(String(plate.price));
      setOldPrice(plate.old_price != null ? String(plate.old_price) : '');
      setDiscount(plate.discount != null ? String(plate.discount) : '');
      setStatus(plate.status);
      setPreview(plate.image_url);
      setImage(null);
      setErrors({});
    }
  }, [plate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await plateApi.update(plate.id, fd);
      onSuccess();
      onClose();
    } catch (err) {
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-2xl bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800 shrink-0">
            <div>
              <h2 className="font-bold text-lg text-gray-900 dark:text-white">Edit Plate</h2>
              <p className="text-xs text-gray-400 mt-0.5">Editing: {plate.name}</p>
            </div>
          ...
          <div className="overflow-y-auto flex-1">
            <form onSubmit={handleSubmit} id="edit-plate-form" className="p-6 space-y-5">
              {errors.api && (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={name} onChange={e => setName(e.target.value)} className={inputCls('name')} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <input type="number" step="0.01" min="0" value={price} onChange={e => setPrice(e.target.value)} className={inputCls('price')} />
                  {errors.price && <p className="text-xs text-red-500 mt-1">{errors.price}</p>}
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Old Price</label>
                  <input type="number" step="0.01" min="0" value={oldPrice} onChange={e => setOldPrice(e.target.value)} className={inputCls('old_price')} />
                ...
                <div>
                  ...
                  <input type="number" min="0" max="100" value={discount} onChange={e => setDiscount(e.target.value)} className={inputCls('discount')} />
              ...
              <div>
                ...
                <textarea rows={2} value={shortDesc} onChange={e => setShortDesc(e.target.value)} className={`${inputCls('short_desc')} resize-none`} />
              ...
              <div>
                ...
                <textarea rows={3} value={desc} onChange={e => setDesc(e.target.value)} className={`${inputCls('desc')} resize-none`} />
          ...
          <div className="flex gap-3 px-6 py-4 border-t border-gray-100 dark:border-gray-800 shrink-0">
            ...
            <button type="submit" form="edit-plate-form" disabled={loading}
