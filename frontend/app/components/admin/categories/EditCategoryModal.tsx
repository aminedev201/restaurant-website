...
export default function EditCategoryModal({ open, category, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await categoryApi.update(category.id, fd);
      onSuccess();
      onClose();
    } catch (err: unknown) {
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-lg bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800 shrink-0">
            <div>
              <h2 className="font-bold text-lg text-gray-900 dark:text-white">Edit Category</h2>
              <p className="text-xs text-gray-400 mt-0.5">Editing: {category.name}</p>
            </div>
          ...
          <div className="overflow-y-auto flex-1">
            <form onSubmit={handleSubmit} id="edit-category-form" className="p-6 space-y-5">
              {errors.api && (
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-2.5 rounded-xl border text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 outline-none transition-colors
                    ${errors.name ? 'border-red-400' : 'border-gray-200 dark:border-gray-700 focus:border-brand-500 dark:focus:border-brand-400'}`}
                />
